...
from src.get_datetime import get_now_timestamp
...
class KitAPIClient:
    ...
    async def _post_request(self, endpoint: str, payload: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        request_id = await get_now_timestamp()
        request_body = self._build_request_body(request_id)
    ...
    def build_filter(
        ...
        company_id = company_id if company_id else self._company_id
        filter_obj = {
            "UpDate": up_date,
            "ToDate": to_date,
    ...
    def _build_request_body(self, request_id: int) -> dict[str, Any]:
